...
def handle_subscription_doubt(update: Update, context: CallbackContext):
    ...
    doubt_text = ("Понимаю, что важно чувствовать уверенность в своем решении\n"
                    "Чего тебе не хатает, тобы дать нам шанс?")
...
def handle_expensive_doubt(update: Update, context: CallbackContext):
    ...
    expensive_text = ("Давай честно:\n"
                      "Это не трата. Это шанс вложиться в то, что\n"
                      "действительно важно — в своё здоровье, в\n"
                      "своё тело, в своё качество жизни.\n"
                      "1 555 ₽ — это всего 50 рублей в день.\n"
                      "Меньше, чем чашка кофе.\n"
                      "А возможности? Они бесценны:\n"
                      "— Лёгкость в теле\n"
                      "— Осанка, которая дарит уверенность\n"
                      "— Спокойная голова и внутрення энергия\n"
                      "Ты освобождаешь своё время от боли,\n"
                      "усталости и хаоса.\n"
                      "И знаешь, идеального момента никогда не\n"
                      "бывает.\n"
                      "Но бывает правильное решение — выбрать\n"
                      "себя.\n"
                      "А мы поможем пройти это")
...
def handle_expensive_no(update: Update, context: CallbackContext):
    ...
    text = ("Ты прав(а), что все тщательно обдумываешь\n"
            "Мы не обещаем чудес за неделю.\n"
            "В WILLWAY мы знаем: настоящие изменения\n"
            "приходят через регулярные маленькие шаги.\n"
            "Без гонки. Без давления. В своём темпе.\n"
            "Иногда это легко и вдохновляет.\n"
            "Иногда — бывают спады, сомнения,\n"
            "усталость. И это нормально.\n"
            "Мы рядом, чтобы помочь тебе начать и не\n"
            "дать сойти с пути.\n"
            "Ты здесь не один.\n"
            "Сотни людей выбрали заботиться о себе\n"
            "вместе с нами, даже сделали отдельный\n"
            "канал с отзывами, загляни ⤵️\n"
            "Ссылка на канал с отзывами\n"
            "Гарантия:\n"
            "- Ты ничего не теряешь, если что-то не\n"
            "понравится - в течение 7 дней просто\n"
            "напишешь в поддержку и мы вернем деньги")
    ...
    keyboard = [
        [InlineKeyboardButton("Да", callback_data=f"{BACK_TO_PLANS_CB}")],
...
def handle_final_no(update: Update, context: CallbackContext):
    ...
    final_text = ("Жаль, что у нас не получится поработать\n"
                  "вместе, но будем очень счастливы честной\n"
                  "обратной связи, почему не дал(а) нам шанс:")
